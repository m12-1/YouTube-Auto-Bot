...
def publish_video(video_path: str, title: str, description: str, tags: list[str],
                   category_id: str = "27", privacy_status: str = "public",
                   contains_synthetic_media: bool = True) -> str:
    """يرفع الفيديو ويعيد video_id.

    contains_synthetic_media: يضبط الحقل الرسمي status.containsSyntheticMedia
    (مدعوم في YouTube Data API v3 منذ 30 أكتوبر 2024) للإفصاح عن أن الفيديو
    يحوي صوتًا اصطناعيًا (TTS) وسكربتًا مولَّدًا بالذكاء الاصطناعي. مضبوط True
    افتراضيًا لتفادي أي التباس مع سياسة "Altered or Synthetic content"، رغم أن
    هذا الحقل مخصص أصلًا للمحتوى الواقعي المضلِّل (مثل انتحال شخص حقيقي) أكثر
    من كونه إلزاميًا لتعليق صوتي على فيديوهات ستوك حقيقية - التفعيل لا يكلّف
    شيئًا ويحمي الحساب من أي مراجعة يدوية مستقبلية.
    """
