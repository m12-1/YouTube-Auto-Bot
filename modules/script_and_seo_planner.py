...
2. scenes: split narration into consecutive scenes, each with:
   ...
   - actor_ref: the SPECIFIC named entity (planet, animal, person, place,
     object...) that must ITSELF be the one performing/undergoing the action
     described in this scene's text, if the text names or clearly implies one
     specific entity (including by pronoun reference to an entity named in an
     earlier scene). Example: if the text is "Mars gets hit by a dust storm",
     actor_ref = "Mars" — it is not enough for a video to show "a planet" and
     "a dust storm" separately; Mars itself must be the one experiencing the
     storm. If the text refers to a category/kind rather than one specific
     individual (e.g. "great white sharks hunt in these waters" — any great
     white shark qualifies, not one particular shark), set actor_ref to the
     general kind/species, not a unique individual. If the text describes a
     generic phenomenon with no specific named entity at all, set actor_ref
     to null.
...
{{
  "narration": "...",
  "scenes": [
    {{"id": "scene_1", "text": "...", "actor_ref": "..." or null, "visual_keywords": ["subject action setting", "subject motion context", "subject detail close-up"], "duration_estimate": 5.0}}
  ],
  "video_title": "...",
  "youtube_keywords": ["...", "..."],
