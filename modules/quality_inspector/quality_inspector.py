...
# Thresholds for the three-tier verdict system:
#   PASS:               ratio ≤ _PASS_MAX_RATIO  (effectively zero missing)
...
def _check_no_missing_scenes(render_plan: Dict[str, Any]) -> Tuple[bool, List[str], Dict[str, Any]]:
    ...
    placeholder clip. Returns additional metadata (missing_scene_count,
    total_scene_count, missing_ratio) for the final report.

    Returns:
        (check_passed, failure_reasons, scene_stats)
    """
    missing = render_plan.get("missing_media_scene_ids", [])
    total_scenes = len(render_plan.get("tracks", {}).get("video", [])) or max(len(missing), 1)
    ratio = len(missing) / total_scenes if total_scenes > 0 else 0.0
    ...
    if not missing:
        return True, [], scene_stats

    if ratio > _WARN_MAX_RATIO:
        return False, [
    ...
    if ratio > _PASS_MAX_RATIO:
        logger.info(
            "quality_inspector: scene(s) %s used a placeholder clip (%d/%d = %.0f%%), "
            "within PASS_WITH_WARNINGS tolerance (%.0f%%-%.0f%%).",
            missing, len(missing), total_scenes, ratio * 100,
            _PASS_MAX_RATIO * 100, _WARN_MAX_RATIO * 100,
        )
        return True, [], scene_stats

    logger.info(
        "quality_inspector: scene(s) %s used a placeholder clip (%d/%d = %.0f%%), "
        "within the clean PASS tolerance (≤%.0f%%).",
        missing, len(missing), total_scenes, ratio * 100,
        _PASS_MAX_RATIO * 100,
    )
    return True, [], scene_stats
...
def run(input_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # _check_no_missing_scenes returns an extra scene_stats dict
        scenes_ok, scenes_reasons, scene_stats = _check_no_missing_scenes(render_plan)
        checks["no_missing_scenes"] = scenes_ok
        failure_reasons.extend(scenes_reasons)
        ...
        for name, check_fn in standard_checks.items():
            passed, reasons = check_fn()
            checks[name] = passed
            failure_reasons.extend(reasons)

        # Two-tier verdict:
        #   FAIL → any check failed or missing scenes exceed _PASS_MAX_RATIO
        #   PASS → all checks passed and missing scenes ≤ _PASS_MAX_RATIO
        if not all(checks.values()):
            verdict = "FAIL"
        elif scene_stats.get("missing_ratio", 0.0) > _PASS_MAX_RATIO:
            verdict = "FAIL"
        else:
            verdict = "PASS"

        logger.info(
            "Quality inspection complete for run_id=%s topic='%s' -> verdict=%s "
            "(missing=%d/%d, %d failure(s))",
            run_id,
            topic,
            verdict,
            scene_stats.get("missing_scene_count", 0),
            scene_stats.get("total_scene_count", 0),
            len(failure_reasons),
        )
        ...
        data = {
            ...
            "missing_scene_count": scene_stats.get("missing_scene_count", 0),
            "total_scene_count": scene_stats.get("total_scene_count", 0),
            "missing_ratio": scene_stats.get("missing_ratio", 0.0),
        }
