...
_VERIFY_PROMPT = """
لدي {n} مقاطع فيديو مرشحة لتمثيل مشهد واحد في فيديو "شورت" قصير.

⚠️ مهم جدًا: النص أدناه ليس مجرد وصف عام للموضوع - هو **النص الحرفي الذي
سيظهر كترجمة مكتوبة على الشاشة (subtitle) وسيُسمع في التعليق الصوتي في نفس
اللحظة الزمنية التي يظهر فيها الفيديو المختار**. أي متفرج سيرى الفيديو وهذا
النص معًا في نفس الإطار. لذا يجب أن تحاكم الفيديو على أساس: "هل هذا الفيديو
تحديدًا هو ما يتوقعه المشاهد رؤيته وهو يقرأ/يسمع بالضبط هذه الكلمات؟"

النص الذي سيظهر متزامنًا مع الفيديو: "{narration_excerpt}"
{actor_ref_context}
...
{candidates_meta}
{prev_scene_context}
منهج الفحص الإلزامي لكل مرشح (بهذا الترتيب بالضبط):

الخطوة 1 - استخراج مستقل: قبل أي مقارنة بالنص، حلّل الفيديو بمعزل تام وحدد
ما تراه فعليًا: من/ما هو الكيان الرئيسي الظاهر (detected_entity، بأكبر دقة
هوية ممكنة، مثال: "كوكب المريخ" لا "كوكب" فقط)، وماذا يفعل، وأين. سجّل هذا
أولًا قبل أن تنظر في مدى ملاءمته للنص.

الخطوة 2 - المقارنة: قارن الآن الكيان الذي استخرجته فعليًا (detected_entity)
مع الكيان الذي يتطلبه النص/actor_ref أعلاه، وصنّف النتيجة حصريًا كواحدة من:
- "exact_match": الكيان المكتشف هو نفسه الكيان المطلوب تحديدًا (نفس الفرد أو
  نفس النوع/الجرم المحدد بالاسم إن كان actor_ref يشير لنوع/جرم معيّن).
- "class_match_only": الكيان المكتشف من نفس الفئة العامة فقط (مثال: "كوكب"
  بينما المطلوب "المريخ" تحديدًا) وليس الكيان نفسه.
- "mismatch": الكيان المكتشف مختلف فعليًا وواضح عن المطلوب.
- "uncertain": لا يمكن تحديد هوية الكيان المكتشف بثقة كافية من الفيديو نفسه
  ولا من عنوانه/وصفه.
لا تسمح لنجاح مطابقة الفعل (action) أو المشهد العام أن يعوّض فشل هذه الخطوة؛
الهوية تُحكَم بمعزل عن مدى إثارة أو ملاءمة بقية المشهد بصريًا.

الخطوة 3 - أدلة سلبية: ابحث عمدًا عن أي سبب يدفعك لرفض هذا المرشح تحديدًا
(وليس فقط أسباب قبوله) - مثال: العنوان/الوصف يذكر موضوعًا مختلفًا، أو
التفاصيل البصرية (لون/شكل/بيئة) لا تطابق الكيان المطلوب رغم تشابه الفئة
العامة. سجّل هذا في negative_evidence (نص مختصر)، أو اتركه فارغًا إن لم تجد
أي سبب فعلي وجاد للرفض بعد بحث حقيقي.

بعد هذه الخطوات الثلاث فقط، إن كان identity_match هو "mismatch" فهذا خطأ
فادح (Entity Mismatch) ويجب إعطاؤه score=0 دائمًا مهما بدا التطابق البصري
العام جذابًا. لا تتساهل هنا حتى لو كان هذا أفضل مرشح متاح.
...
- الشك يُفسَّر دائمًا ضد المرشح (benefit of the doubt يذهب للرفض لا للقبول):
  إن لم تكن متأكدًا تمامًا من التطابق، صنّف identity_match كـ "uncertain"
  وأعطِ درجة منخفضة صراحة بدل تقريب الدرجة للأعلى تفاؤلاً.
...
لكل فيديو (بالترتيب الذي أُرسل به)، وبعد تطبيق الخطوات الثلاث أعلاه، قيّم
مدى ملاءمته من 0 إلى 10 لتمثيل هذا النص المتزامن بصريًا، وحدد أفضل نطاق زمني
(بالثواني) داخل الفيديو يمثل جوهر المشهد (لا يتجاوز طول المقطع الفعلي). أعد
أيضًا لكل مرشح ملخصًا بصريًا مختصرًا جدًا (subject/action/setting) يصف ما
يظهر فعليًا في الفيديو، بصرف النظر عن درجته، ليُستخدم عند تقييم المشهد التالي.

أعد **فقط** كائن JSON بهذا الشكل، بدون أي نص إضافي:
{{
  "results": [
    {{"candidate_index": 0, "detected_entity": "...",
      "identity_match": "exact_match|class_match_only|mismatch|uncertain",
      "negative_evidence": "...", "score": 0,
      "best_segment": {{"start": 0.0, "end": 0.0}},
      "visual_summary": {{"subject": "...", "action": "...", "setting": "..."}}}}
  ]
}}
"""

_ACTOR_REF_CONTEXT_TEMPLATE = """
تحديد صريح للكيان المطلوب من عقد المشهد (لا تعتمد فقط على استنتاجك من النص):
الكيان الذي يجب أن يكون هو نفسه من يقوم/يمر بالفعل في هذا المشهد تحديدًا هو:
"{actor_ref}". لا يكفي ظهور فعل مطابق مع كيان آخر من نفس الفئة العامة؛ يجب أن
يكون هذا الكيان تحديدًا هو محور الفيديو.
"""

_PREV_SCENE_CONTEXT_TEMPLATE = """
ذاكرة آخر مشهد مقبول قبل هذا المشهد مباشرة في نفس الفيديو (هذه ذاكرة مساعدة
فقط للاتساق - وقد تكون هي نفسها خاطئة، فلا تعتمد عليها وحدها كمرجع مطلق؛
المرجع الأقوى دائمًا هو عقد المتطلبات ونص المشهد الحالي نفسه):
{prev_summary_json}

أولًا: صنّف بنفسك نوع العلاقة المطلوبة بين هذا المشهد والمشهد السابق، بالنظر
فقط إلى نص المشهد الحالي (وactor_ref أعلاه إن وُجد)، إلى واحدة من ثلاث حالات:
1. "same_entity": النص الحالي يفترض صراحة استمرار الحديث عن نفس الكيان/الفرد
   بعينه من المشهد السابق (ضمير عائد، إشارة صريحة، أو استكمال منطقي واضح لنفس
   الفرد المحدد). في هذه الحالة فقط يجب أن يكون الكيان المكتشف في المرشح
   الحالي هو نفس الفرد (أو على الأقل نفس النوع/الجرم المحدد) الذي ظهر في
   المشهد السابق.
2. "same_type": النص يتحدث عن نفس النوع/الفئة العامة (وليس بالضرورة نفس
   الفرد بعينه) - مثال: مشهد سابق عن سمكة قرش بيضاء تسبح، ومشهد حالٍ عن سمكة
   قرش بيضاء أخرى تصطاد. هنا يكفي أن يكون الكيان المكتشف من نفس النوع، دون
   اشتراط أنه نفس الفرد حرفيًا.
3. "different_subject": النص الحالي ينتقل عمدًا لكيان أو فكرة مختلفة تمامًا
   عن المشهد السابق (حتى لو كانت هناك مقارنة أو علاقة بينهما، مثل "بالمقارنة
   مع الأرض..."). في هذه الحالة تجاهل تمامًا هوية المشهد السابق ولا تعاقب
   المرشح لاختلافه عنها.

ثانيًا: طبّق القرار بحسب التصنيف الذي اخترته:
- إن كانت "same_entity" واختلف الكيان المكتشف في المرشح الحالي عن كيان
  المشهد السابق (فرد أو جرم مختلف فعليًا، وليس مجرد زاوية تصوير مختلفة)، فهذا
  خطأ اتساق فادح يُعامَل مثل Entity Mismatch تمامًا: اجعل identity_match
  لهذا المرشح "mismatch" وscore=0، حتى لو كان مطابقًا جيدًا لنص مشهده وحده
  بمعزل عن السياق.
- إن كانت "same_type" ولم يكن المرشح من نفس النوع العام، عاملها أيضًا كخطأ
  اتساق (identity_match="mismatch").
- إن كانت "different_subject"، لا تستخدم هذا السياق إطلاقًا في تقييمك.
"""
...
لدي {n} صور مرشحة (خطة بديلة صارمة بعد فشل إيجاد فيديو مناسب) لتمثيل مشهد
واحد في فيديو "شورت" قصير.

⚠️ مهم جدًا: النص أدناه هو **النص الحرفي الذي سيظهر كترجمة مكتوبة على الشاشة
وسيُسمع في التعليق الصوتي في نفس اللحظة التي تُعرض فيها هذه الصورة**. المشاهد
سيرى الصورة وهذا النص معًا في نفس الإطار، لذا احكم على الصورة على أساس: "هل
هذه الصورة تحديدًا هي ما يتوقعه المشاهد رؤيته وهو يقرأ/يسمع بالضبط هذه الكلمات؟"

النص الذي سيظهر متزامنًا مع الصورة: "{narration_excerpt}"
...
بالاسم (كوكب، جرم سماوي، شخص، مكان، كائن...) وكانت الصورة (بصريًا، أو حسب
...
فعلي واضح لا لبس فيه).

لكل صورة (بالترتيب الذي أُرسلت به)، قيّم مدى ملاءمتها من 0 إلى 10 لتمثيل هذا
...
def _apply_identity_decision(results: list[dict], actor_ref: str | None) -> list[dict]:
    """يطبّق قرار الهوية في الكود لا في Gemini وحده (نقطة 3: "القرار يتخذه
    الكود"): عدم اليقين أو التطابق الخاطئ = رفض دائمًا (score=0)، وحين يتطلب
    عقد المشهد كيانًا محددًا (actor_ref)، لا يكفي تطابق الفئة العامة فقط."""
    for r in results:
        identity_match = r.get("identity_match")
        if identity_match in ("mismatch", "uncertain", None):
            if r.get("score", 0) != 0:
                logger.info(
                    "[قرار الهوية] فرض score=0 على مرشح %s (identity_match=%s كان score=%s).",
                    r.get("candidate_index"), identity_match, r.get("score"),
                )
            r["score"] = 0
        elif identity_match == "class_match_only" and actor_ref:
            if r.get("score", 0) != 0:
                logger.info(
                    "[قرار الهوية] فرض score=0 على مرشح %s (تطابق فئة عامة فقط، لكن actor_ref='%s' يتطلب هوية دقيقة).",
                    r.get("candidate_index"), actor_ref,
                )
            r["score"] = 0
    return results
...
def _verify_batch(narration_excerpt: str, candidates: list[dict], local_paths: list[str],
                   prev_visual_memory: dict | None = None, actor_ref: str | None = None) -> list[dict]:
    prev_context = ""
    if prev_visual_memory:
        prev_context = _PREV_SCENE_CONTEXT_TEMPLATE.format(
            prev_summary_json=json.dumps(prev_visual_memory, ensure_ascii=False)
        )
    actor_ref_context = ""
    if actor_ref:
        actor_ref_context = _ACTOR_REF_CONTEXT_TEMPLATE.format(actor_ref=actor_ref)
    prompt = _VERIFY_PROMPT.format(
        n=len(candidates),
        narration_excerpt=narration_excerpt,
        actor_ref_context=actor_ref_context,
        candidates_meta=_format_candidates_meta(candidates),
    ...
    results = parse_json_response(raw).get("results", [])
    return _apply_identity_decision(results, actor_ref)
...
def verify_scene_media(scene: dict, run_state: RunState, media_dir: str, category: str = "", topic: str = "") -> dict | None:
    ...
    # عقد المشهد الثابت (منذ اعتماد السكربت) - نستخدم actor_ref منه إن وُجد
    # بدل الاعتماد فقط على استنتاج Gemini اللحظي من النص وحده.
    contract = run_state.get_scene_contract(scene_id) or {}
    actor_ref = contract.get("actor_ref") or scene.get("actor_ref")
    ...
    for retry in range(MAX_KEYWORD_RETRY_PER_SCENE):
        ...
        prev_visual_memory = run_state.get_previous_scene_visual_memory(scene_id)
        try:
            results = _verify_batch(narration_excerpt, top3, preview_paths, prev_visual_memory, actor_ref)
        except Exception as e:  # noqa: BLE001
        ...
        # سجل verification_attempts.json: كل مرشح قُيِّم فعليًا، مقبولًا كان أو
        # مرفوضًا، مع سبب القرار (هوية/درجة)، لغرض التدقيق اللاحق.
        for r in results:
            r_idx = r.get("candidate_index")
            cand_meta = top3[r_idx] if r_idx is not None and 0 <= r_idx < len(top3) else {}
            run_state.log_verification_attempt(scene_id, {
                "keyword": cand_meta.get("keyword"),
                "source": cand_meta.get("source"),
                "candidate_id": cand_meta.get("id"),
                "detected_entity": r.get("detected_entity"),
                "identity_match": r.get("identity_match"),
                "negative_evidence": r.get("negative_evidence"),
                "score": r.get("score"),
                "accepted": r.get("score", 0) >= MEDIA_RELEVANCE_ACCEPT_THRESHOLD,
            })
        ...
        if best is not None:
            ...
            run_state.set_scene_visual_memory(scene_id, {
                "visual_summary": best.get("visual_summary"),
                "detected_entity": best.get("detected_entity"),
                "identity_match": best.get("identity_match"),
            })
            if ENABLE_CROSS_SCENE_MEDIA_DEDUP:
    ...
    if fallback_best is not None:
        ...
        run_state.set_scene_visual_memory(scene_id, {
            "visual_summary": fb_visual_summary,
