"""
الإعدادات المركزية للمشروع.
"""

# ===== سلسلة نماذج Gemini المعتمدة (من الأفضل إلى الأقل) — جيل 3 فقط =====
MODEL_CHAIN = [
    "gemini-3.6-flash",
    "gemini-3.5-flash",
    "gemini-3.5-flash-lite",
    "gemini-3.1-flash-lite",
    "gemini-3-flash-preview",
]

# ===== تخصيص كل مرحلة بمفتاح Gemini أساسي =====
STAGE_KEY_MAP = {
    "topic_selector":           "GEMINI_KEY_LIGHT",
    "script_and_seo_planner":   "GEMINI_KEY_ADVANCED",
    "ai_media_verification":    "GEMINI_KEY_FILTER",
    "final_scene_audit":        "GEMINI_KEY_FILTER_2",
    "graphic_fallback":         "GEMINI_KEY_LIGHT",     # صياغة نص بطاقة الملاذ الأخير (ترجمة/تكثيف قصير) - مهمة بسيطة
    "competitor_seo_optimizer": "GEMINI_KEY_IMAGE",
    # مرحلتا التخطيط (planner/) - إضافة تكميلية فقط، لا تمس مراحل الإنتاج أعلاه
    "content_planner":          "GEMINI_KEY_LIGHT",     # بناء خطة أسبوعية/شهرية - مهمة بسيطة
    "category_analyzer":        "GEMINI_KEY_ADVANCED",  # تحليل أداء الفئات - يحتاج استدلالًا أعمق
}

# ترتيب المفاتيح الاحتياطي العالمي (يُستخدم عند نفاد كل نماذج المفتاح الأساسي للمرحلة)
ALL_KEYS_ORDER = [
    "GEMINI_KEY_ADVANCED",
    "GEMINI_KEY_FILTER",
    "GEMINI_KEY_FILTER_2",
    "GEMINI_KEY_IMAGE",
    "GEMINI_KEY_LIGHT",
]

# ===== حدود الفيديو =====
MIN_VIDEO_SECONDS = 30
MAX_VIDEO_SECONDS = 60
EXPORT_RESOLUTION = (1080, 1920)  # عمودي لليوتيوب شورتس
FADE_DURATION = 0.25              # ثواني، انتقال تلاشي (crossfade) سريع

# ===== تمديد المشاهد الأقصر من زمن النطق المطلوب (بديل التجميد/vfx.freeze) =====
# عندما يكون طول لقطة المصدر أقصر من audio_duration المطلوب لهذا المشهد، كان
# الكود سابقًا "يجمّد" آخر إطار (freeze) حتى اكتمال المدة، وهو ما يُنتج إحساس
# "توقف الشاشة" الملحوظ عند بعض المشاهد. البديل الآن مرتّب حسب الأفضلية:
# إبطاء بسيط للسرعة، ثم بومرانج (أمام/عكس) لو كانت الفجوة أكبر، ولا يُستخدم
# التجميد إلا كملاذ أخير لمصدر قصير جدًا لا يصلح لأي منهما.

# أعلى نسبة استطالة (audio_duration / طول اللقطة الأصلي) نقبل عندها إبطاء
# السرعة مباشرة بدل اللجوء لبومرانج. 1.4 تعني: نقبل إبطاءً حتى 40% لأنه غير
# ملحوظ للعين في لقطات B-roll القصيرة؛ أكثر من ذلك تصبح الحركة بطيئة وغير
# طبيعية بصريًا فنفضّل البومرانج بدلاً منه.
MAX_CLIP_SLOWDOWN_STRETCH_RATIO = 1.4

# أقل طول للقطة مصدر (بالثواني) يمكن بناء بومرانج مفيد/غير مزعج منه. لقطة
# أقصر من هذا (أقل من ثانية) ستُنتج بومرانج سريعًا جدًا ومزعجًا للعين، فنلجأ
# للتجميد كملاذ أخير نادر في هذه الحالة فقط.
MIN_SOURCE_SECONDS_FOR_BOOMERANG = 1.0

# ===== منع تكرار نفس مقطع المصدر (نفس الفيديو) في أكثر من مشهد بالفيديو الواحد =====
# لو تعطّل هذا الخيار (False)، يعود السلوك لسابقه: كل مشهد يُقيَّم بمعزل عن
# باقي المشاهد، فقد يختار نفس الفيديو المصدر (نفس source+id) الذي اختير فعلاً
# لمشهد آخر سابق في نفس الفيديو النهائي — وهذا هو سبب "تكرار المشهد" الملحوظ.
ENABLE_CROSS_SCENE_MEDIA_DEDUP = True

# ===== عتبات التقييم =====
MEDIA_RELEVANCE_ACCEPT_THRESHOLD = 9       # من 10 - قبول المشهد من نتائج البحث
# رُفعت من 7.5 إلى 9 لتطابق الهدف الفعلي المعلن (كل مشهد 9/10 حقيقي)، بدل
# قبول مشاهد بتقييم 7.5-8.9 كـ"ناجحة" رغم أنها دون الهدف. رفع هذه العتبة كان
# سيزيد عدد المشاهد التي تحتاج "مراجعة يدوية" بسبب سقف واقعي في مصادر
# الفيديو/الصور المجانية لبعض المشاهد (خصوصًا حقائق رقمية/مفاهيمية) — لهذا
# أصبح رفعها آمنًا فقط بعد إضافة طبقة "الملاذ الأخير المضمون"
# (modules/graphic_fallback.py) التي تضمن مخرجًا دائمًا بدرجة 9.2 لأي مشهد
# ما زال دون هذه العتبة بعد استنفاد كل محاولات إعادة البحث.
SCENE_AUDIT_ACCEPT_THRESHOLD = 9           # من 10 - قبول المشهد في التدقيق النهائي

# من 10 - قبول الهوك (أول 2-3 ثوانٍ من النص) ضمن نفس استدعاء بناء السكربت
# (المرحلة 3)، عبر تقييم Gemini الذاتي لما كتبه (self-critique) في نفس
# استجابة JSON، دون أي استدعاء إضافي. لو كان الهوك دون هذه العتبة، يُطلب من
# Gemini في المحاولة التالية (نفس حلقة إعادة الضبط المستخدمة أصلاً لضبط
# المدة) إعادة كتابة الهوك فقط بما يعالج الملاحظة المذكورة ويناسب الموضوع.
HOOK_QUALITY_ACCEPT_THRESHOLD = 7.5
MAX_CANDIDATES_PER_VERIFICATION_BATCH = 5  # عدد الفيديوهات المرسلة دفعة واحدة للتقييم (رُفعت من 3 بعد إضافة 3 مصادر جديدة)

# ===== حدود إعادة المحاولة =====
MAX_KEYWORD_RETRY_PER_SCENE = 2   # عدد محاولات تبديل الكلمة المفتاحية لكل مشهد (نقطة 5)
MAX_SCENE_AUDIT_RETRIES = 3       # عدد محاولات إعادة بناء مشهد مرفوض (نقطة 7)

# الحد الأدنى المطلق المقبول لخطة fallback (أفضل مرشح لم يبلغ عتبة القبول
# العادية MEDIA_RELEVANCE_ACCEPT_THRESHOLD). أي مرشح fallback بدرجة أقل من هذا
# الحد لا يُقبل بصمت، بل يُعلَّم المشهد بعلم needs_manual_review.
MIN_FALLBACK_ACCEPT_SCORE = 7

# ===== خطة الصور البديلة (Fallback) عند فشل إيجاد فيديو مناسب لمشهد =====
# تُستخدم فقط بعد استنفاد كل محاولات الفيديو العادية (MAX_KEYWORD_RETRY_PER_SCENE)
# دون بلوغ عتبة القبول. الشرط صارم جدًا (أعلى من عتبة الفيديو نفسها) لأن الصورة
# الواحدة أقل ديناميكية من الفيديو، فيجب أن تكون مطابقتها للنص شبه مثالية
# لتعويض ذلك، وإلا نُفضّل الرجوع لمنطق fallback الفيديو القديم/المراجعة اليدوية
# بدل نشر صورة غير مطابقة فعليًا.
MEDIA_IMAGE_RELEVANCE_ACCEPT_THRESHOLD = 9   # من 10 - مساوية لعتبة الفيديو (9) بعد توحيدهما
MIN_IMAGES_PER_SCENE = 3    # أقل عدد صور مقبولة (score>=9) لتغطية مدة المشهد
MAX_IMAGES_PER_SCENE = 4    # أعلى عدد صور تُعرض بالتتابع لتغطية مشهد واحد
MAX_IMAGE_KEYWORD_RETRY_PER_SCENE = 4  # محاولات تبديل الكلمة المفتاحية أثناء البحث عن صور
IMAGE_KEN_BURNS_ZOOM_RATIO = 1.12  # نسبة تكبير الزوم البطيء على مدى عرض كل صورة (12%)

# ===== الفئات التي يجب جلب فيديوهاتها من ناسا (فضاء/فلك) =====
SPACE_ASTRONOMY_CATEGORY_KEYWORDS = [
    "فضاء", "فلك", "كواكب", "نجوم", "مجرة", "مجرات", "كون",
    "space", "astronomy", "planet", "planets", "galaxy", "galaxies",
    "universe", "nasa", "cosmos", "star", "stars",
]

# ===== مجموعات كيانات متضاربة (لمنع خلط الهوية، مثل ظهور بلوتو بدل الزهرة) =====
# كل مجموعة فرعية تمثّل أسماء بديلة لنفس الكيان. أي عنصر نصي (عنوان/وصف) يذكر
# كيانًا من نفس المجموعة الكبرى لكن بمجموعة فرعية مختلفة عن الكلمة المفتاحية
# المستهدفة يُعتبر عدم تطابق ويُستبعد المرشح (البحث يجب أن يكون موجّهًا للنتيجة
# بدقة، لا مجرد تشابه لفظي/موضوعي عام). تُفحص فقط المصادر التي توفّر نصًا
# (عنوان/وصف) فعليًا: Internet Archive وWikimedia Commons وNASA؛
# Pexels/Pixabay بلا عناوين فتُترك لفلتر التحقق البصري (Gemini).
# مثال حقيقي دفع لإضافة هذا: البحث عن "Venus/الزهرة" أعاد فيديو بلوتو من NASA.
CONFLICTING_ENTITY_GROUPS = [
    [
        ["mercury", "عطارد"],
        ["venus", "venusian", "الزهرة"],
        ["earth", "الأرض"],
        ["mars", "martian", "المريخ"],
        ["jupiter", "المشتري"],
        ["saturn", "زحل"],
        ["uranus", "أورانوس"],
        ["neptune", "نبتون"],
        ["pluto", "بلوتو", "tombaugh regio", "new horizons"],
        ["moon", "lunar", "القمر"],
    ],
]

# ===== عدد المقاطع المطلوب جلبها من كل مصدر لكل كلمة مفتاحية =====
CANDIDATES_PER_KEYWORD_PER_SOURCE = 3   # نقطة 4: لكل كلمة 3 مقاطع من كل مصدر

# الحد الأدنى لدقة الفيديو (الارتفاع بالبكسل) المقبول من أي مصدر - نفس الحد
# المطبّق على Pexels/Pixabay، ويُطبَّق أيضًا على Internet Archive وWikimedia
# Commons لتفادي المسحات القديمة/الملفات منخفضة الجودة.
MIN_ACCEPTABLE_VIDEO_HEIGHT = 1080

# ===== فلتر اتجاه الفيديو (عمودي/أفقي) =====
# القيم الممكنة: "portrait" (عمودي، ارتفاع > عرض) و"landscape" (أفقي، عرض >
# ارتفاع). يُطبَّق هذا الفلتر على كل المصادر (Pexels، Internet Archive،
# Wikimedia Commons، NASA) باستثناء Pixabay تحديدًا.
#
# Pixabay مستثنى لأنه لا يدعم تحديد الاتجاه عبر الـ API، ويُرجع فيديوهاته
# عالية الدقة (tier "large") أفقية شبه دائمًا (مثال: 1920x1080)؛ تقييده بهذا
# الفلتر كان يُصفّر نتائجه بالكامل. الاستثناء الخاص به مُعرَّف مباشرة في
# media_downloader.py عبر _PIXABAY_ALLOWED_ORIENTATIONS (يقبل كل الاتجاهات)،
# وليس هنا، حتى لا يؤثر تغيير هذا الإعداد على Pixabay بالخطأ مستقبلاً.
ALLOWED_VIDEO_ORIENTATIONS = ["portrait"]

# ===== يوتيوب SEO =====
TOP_COMPETITOR_VIDEOS = 5
