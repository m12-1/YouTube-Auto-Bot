...
import { Img, interpolate, useCurrentFrame, staticFile, Easing } from "remotion";
...
export const KenBurnsImage: React.FC<Props> = ({
  ...
  if (pattern === 0) {
    translateX = interpolate(progress, [0, 1], [0, -panLimit], { easing: Easing.inOut(Easing.ease) });
  } else if (pattern === 1) {
    scale = interpolate(progress, [0, 1], [zoomEnd, zoomStart], { easing: Easing.inOut(Easing.ease) });
    translateY = interpolate(progress, [0, 1], [-10, 0], { easing: Easing.inOut(Easing.ease) });
  } else if (pattern === 2) {
    translateX = interpolate(progress, [0, 1], [panLimit, -panLimit], { easing: Easing.inOut(Easing.ease) });
    translateY = interpolate(progress, [0, 1], [-8, 8], { easing: Easing.inOut(Easing.ease) });
  } else {
    translateX = interpolate(progress, [0, 1], [0, panLimit], { easing: Easing.inOut(Easing.ease) });
  }
