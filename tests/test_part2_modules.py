...
def test_video_renderer_falls_back_to_manifest_when_media_missing(tmp_path, monkeypatch) -> None:
    ...
    in_project_tmp_dir = PROJECT_ROOT / "output" / "test_tmp_video_renderer"
    in_project_tmp_dir.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(pipeline_config, "VIDEO_OUTPUT_DIR", str(in_project_tmp_dir))
