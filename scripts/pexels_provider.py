...
def fetch_images(keyword: str, per_page: int = 3, orientation: str = "landscape", negative_tags: list[str] = None) -> list[str]:
    ...
    try:
        ...
        for p in photos:
            ...
            p_alt = p.get("alt", "").lower()
            if negative_tags and any(nt in p_url or nt in p_alt for nt in negative_tags):
                continue
            if p.get("src", {}).get("large2x"):
                valid_urls.append(p["src"]["large2x"])
        return valid_urls
...
def fetch_videos(keyword: str, per_page: int = 3, is_short: bool = True, negative_tags: list[str] = None) -> list[dict]:
    """
    ...
    try:
        ...
        for v in videos:
            ...
            if negative_tags and any(nt in v_url for nt in negative_tags):
                continue
            ...
            # نفضّل hd ثم أي ملف بدقة ≥ 1080
            chosen = next((f for f in files if f.get("quality") == "hd"), None)
            if not chosen:
                chosen = next((f for f in files if min(f.get("width", 0), f.get("height", 0)) >= 720), None)
            if not chosen:
                chosen = files[0]
            if not chosen.get("link"):
                continue
            # فلتر الدقة
            c_w, c_h = chosen.get("width", 0), chosen.get("height", 0)
            if c_w and c_h and min(c_w, c_h) < 720:
                continue
            results.append({
                "url": chosen["link"],
