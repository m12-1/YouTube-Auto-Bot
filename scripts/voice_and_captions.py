...
async def _generate(text: str, audio_out_path: str, captions_out_path: str):
    ...
    word_events = []
    # سنقوم بجمع الكلمات يدوياً كخطة بديلة إذا فشل الـ WordBoundary
    words = text.split() 
    ...
    # إذا فشل التوقيت، نولد توقيت اصطناعي (توزيع متساوٍ)
    if not word_events and words:
        total_duration_ms = 55000  # تقدير لـ 55 ثانية
...
def map_scenes_to_timing(scene_narrations: list[str], word_events: list[dict],
    ...
    # إذا لم توجد توقيتات حقيقية، نوزع المشاهد بالتساوي على الفيديو
    if n_words_total == 0:
