...
from scripts import parallel_scene_fetcher

# ملاحظة: ثوابت الاحتياط العام (GENERIC_LAST_RESORT_KEYWORDS) و MAX_GEMINI_REPLACEMENTS
# أُزيلت من هنا — كل منطق اختيار المرشحين وإعادة توليد الكلمات المفتاحية
# أصبح مسؤولية parallel_scene_fetcher.py حصراً (راجع _resolve_single_unit).
# الدالة القديمة _resolve_visual_units أدناه مُبقاة للتوثيق فقط.
...
def _check_resolution(file_path: str, media_type: str) -> bool:
    """يتحقق أن دقة الوسيط لا تقل عن config.MIN_ALLOWED_RESOLUTION.
    يرجع True لو الدقة مقبولة، False لو منخفضة جداً."""
    try:
        if media_type == "video":
            # نستخدم ffprobe لقراءة أبعاد الفيديو
            cmd = [
                "ffprobe", "-v", "error",
                "-select_streams", "v:0",
            ...
            if result.returncode == 0 and "x" in result.stdout.strip():
                ...
                min_dim = min(w, h)
                if min_dim < 720:  # حد أدنى مرن
                    print(f"[RESOLUTION] فيديو {os.path.basename(file_path)} دقته {w}x{h} — أقل من الحد الأدنى. سيتم رفضه.")
                    return False
        else:
            ...
            with Image.open(file_path) as img:
                ...
                min_dim = min(w, h)
                if min_dim < 720:
                    print(f"[RESOLUTION] صورة {os.path.basename(file_path)} دقتها {w}x{h} — أقل من الحد الأدنى. سيتم رفضها.")
                    return False
    except Exception as e:
        print(f"[RESOLUTION WARNING] تعذر فحص دقة {file_path}: {e}. سيتم قبوله.")
    return True
...
def _resolve_visual_units(visual_units: list[dict], workdir: str, file_prefix: str,
    ...
    """
    [غير مُستخدَمة حالياً — أُبقيت فقط للتوثيق/الرجوع إليها]
    استُبدلت بـ parallel_scene_fetcher.resolve_visual_units_parallel التي
    تحل كل الوحدات البصرية بالتوازي (بحث متزامن عبر كل المزودين المتاحين
    لعدة مشاهد بنفس الوقت) بدل معالجتها هنا بالتسلسل الصارم واحدة تلو
    الأخرى.
    ...
    for i, unit in enumerate(visual_units):
        ...
        while True:
            ...
            for item in media_list:
                ...
                # فحص الدقة — رفض الملفات المنخفضة
...
def build_short_montage(short_script: dict, narration_text: str, topic: str, workdir: str) -> dict:
    ...
    media_items = parallel_scene_fetcher.resolve_visual_units_parallel(
        visual_units, workdir, file_prefix="short_scene", is_short=True,
        topic_context=short_topic_context, verify=True,
    )
...
def build_long_montage(long_script: dict, narration_text: str, topic: str, workdir: str) -> dict:
    ...
    تحديث: verify=True الآن (بعد أن كانت False) — بناءً على طلب صريح بألا
    يمر أي مشهد خام بدون تحليل تطابق بصري، تماشياً مع مسار الشورت. كل وحدة
    بصرية تمر الآن عبر نفس سلسلة التحليل الكاملة (Gemini/Groq المتوازيان
    → Puter → CLIP → قبول تلقائي عبر analysis_engine.py)، وتُطلب بدائل من
    Gemini لو رُفضت كل المرشحات المتاحة (نفس سلوك مسار الشورت بالضبط). الفرق
    المكتسب تلقائياً هنا: كل مشهد قد يتحول لعدة وحدات بصرية بمدد مختلفة بدل
    صورة ثابتة واحدة تغطي كل مدته (نفس إصلاح مسار الشورت)، وتوزيع جلب
    الوسائط بين Pixabay و Pexels.
    """
