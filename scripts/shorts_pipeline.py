...
def run():
    ...
    try:
        ...
        urls = asset_fetcher.get_images_for_scene(
            short_script["visual_keywords"], target_count=4, is_short=True
        )
        for j, url in enumerate(urls):
            path = f"{WORKDIR}/short_scene_{j}.jpg"
            downloaded = asset_fetcher.download_image(url, path)
            if downloaded:  # يتجاهل أي صورة فشل تحميلها فعلياً بدل إضافة مسار فارغ
                image_paths.append(downloaded)

        if not image_paths:
            send_alert("توقف إنتاج الشورت: فشل تحميل كل الصور المتاحة.", level="error")
            return
