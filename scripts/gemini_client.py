...
def _verify_media_internal(image_path: str, narration: str, model: str, key_type: str) -> bool:
    ...
    with Image.open(image_path) as img:
        # Pass the image and prompt as a list to Gemini
        response = client.models.generate_content(
            model=model,
            contents=[prompt, img],
            config={"temperature": 0.1} # Low temperature for more deterministic YES/NO
        )
...
def verify_media_relevance(image_path: str, narration: str) -> bool:
    ...
    تسقط تلقائياً إلى الموديل الخفيف إذا فشل الموديل المتقدم.
    """
    try:
        return _verify_media_internal(image_path, narration, MODEL_TEXT_ADVANCED, "filter")
    except Exception as e:
        error_str = str(e).lower()
        if ("429" in error_str or "503" in error_str) and config.GEMINI_KEY_LIGHT:
            reason = "نفاد الحصة (429)" if "429" in error_str else "ازدحام السيرفر (503)"
            print(f"[GEMINI FALLBACK] {reason} في التحقق البصري. الانتقال للموديل الخفيف ({MODEL_TEXT_LIGHT})...")
            # fallback to light model, using filter key (which defaults to advanced) or light key
            return _verify_media_internal(image_path, narration, MODEL_TEXT_LIGHT, "light")
        print(f"[GEMINI ERROR] فشل التحقق البصري: {e}")
        return True # Fallback to True if vision check completely fails so we don't break the pipeline
