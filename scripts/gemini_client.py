...
_clients = {}
_client_keys = {}   # key_type -> api_key string (قاموس عكسي مستقل عن SDK)


def _get_api_key(key_type: str) -> str:
    """يرجع الـ API key المرتبط بهذا الـ key_type — بديل مستقر لـ
    client.api_key الذي لا يوجد كخاصية عامة في genai.Client (خطأ
    AttributeError المتكرر). هذا القاموس نملأه بأنفسنا عند إنشاء العميل
    في _get_client() ولا يعتمد على أي خاصية داخلية للمكتبة."""
    api_key = _client_keys.get(key_type)
    if api_key is None:
        # القيمة لم تُملأ بعد — نُجبر إنشاء العميل أولاً
        _get_client(key_type)
        api_key = _client_keys.get(key_type, "")
    return api_key
...
def _get_client(key_type: str) -> genai.Client:
    ...
    _client_keys[key_type] = api_key   # نحفظ المفتاح بقاموسنا المستقل
    return client
...
def verify_media_relevance(image_path: str, narration: str, topic_context: str = "",
    ...
    while True:
        ...
        if not acquired:
            ...
            valid_models = [(m, k) for m, k in valid_models if not is_key_exhausted(_get_api_key(k))]
            if not valid_models:
...
def score_media_relevance_batch(items: list, topic_context: str = "",
    ...
    models_to_try = _build_filter_cascade(only_key_type)
    valid_models = [(m, k) for m, k in models_to_try if not is_key_exhausted(_get_api_key(k))]
    if not valid_models:
