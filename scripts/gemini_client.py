...
def _build_filter_cascade() -> list:
    """يبني قائمة (model, key_type) الكاملة: 5 نماذج على المفتاح الأساسي
    'filter'، ثم -لو كان GEMINI_KEY_FILTER_2 معرّفاً بالأسرار- نفس الـ5
    نماذج مجدداً على المفتاح الثاني 'filter2' (حصة يومية منفصلة تماماً).
    هذا يضاعف عملياً عدد المحاولات قبل اعتبار Gemini مستنفداً بالكامل
    والتحول لـ Puter."""
...
def verify_media_relevance(image_path: str, narration: str) -> bool:
    ...
    models_to_try = _build_filter_cascade()
