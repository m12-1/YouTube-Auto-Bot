...
def verify_media_relevance(file_path: str, narration: str, topic_context: str = "") -> bool:
    ...
    try:
        ...
        try:
            # Fallback mechanism: Try with `images`, if fails with TypeError, try `image_urls` or `files`
            try:
                response = ai.chat(prompt=prompt, images=[image_path])
            except TypeError as te:
                if "got an unexpected keyword argument 'images'" in str(te):
                    print("[PUTER] Trying fallback for image parameter...")
                    try:
                        response = ai.chat(prompt=prompt, image_urls=[image_path])
                    except TypeError:
                        response = ai.chat(prompt=prompt, files=[image_path])
                else:
                    raise te
        except Exception as e:
