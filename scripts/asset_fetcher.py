...
def _videos_from_provider(provider: str, keywords: list[str], target_count: int, is_short: bool = True,
                           exclude_urls: set = None, negative_tags: list[str] = None) -> tuple[list[dict], float]:
    """يرجع (نتائج الفيديو, أفضل درجة تطابق وُجدت). Pexels لا يملك درجة
    تطابق موثوقة فنعيد 1.0 له إذا وجد أي نتيجة (نثق بترتيب بحثه).
    exclude_urls: روابط مرفوضة سابقاً لهذه الوحدة البصرية — تُستبعد من
    النتائج حتى لو رجعت من نفس المزود (نطلب "غير الذي جلبه أول مرة")."""
    negative_tags = negative_tags or []
    if provider == "pixabay":
        best_score, best_vids = -1.0, None
        for kw in keywords:
            vids = pixabay_provider.fetch_videos(kw, per_page=target_count, is_short=is_short, negative_tags=negative_tags)
            vids = [v for v in vids if v["url"] not in exclude_urls]
    ...
    if provider == "pexels":
        for kw in keywords:
            vids = pexels_provider.fetch_videos(kw, per_page=target_count, is_short=is_short, negative_tags=negative_tags)
            vids = [v for v in vids if v["url"] not in exclude_urls]
            if vids:
                return vids, 1.0
        return [], -1.0
...
def _try_videos_across_providers(keywords: list[str], target_count: int, providers: list[str], is_short: bool = True,
                                  exclude_urls: set = None, negative_tags: list[str] = None) -> list[dict]:
    import concurrent.futures
    all_videos = []
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(providers)) as executor:
        futures = {
            executor.submit(_videos_from_provider, provider, keywords, target_count, is_short, exclude_urls, negative_tags): provider
            for provider in providers
        }
        for future in concurrent.futures.as_completed(futures):
            provider = futures[future]
            try:
                vids, _ = future.result()
                for v in vids:
                    all_videos.append({"type": "video", "url": v["url"], "provider": provider, "score": v.get("score", 1.0)})
            except Exception as e:
                print(f"[ASSET ERROR] Error fetching from {provider}: {e}")

    # Rank them across providers based on score
...
def _try_images_across_providers(keywords: list[str], target_count: int, is_short: bool,
                                  providers: list[str], exclude_urls: set = None, negative_tags: list[str] = None) -> list[dict]:
    import concurrent.futures
    ...
    def _fetch_images(provider: str) -> list[str]:
        images = []
        if provider == "pixabay":
            for kw in keywords:
                images += pixabay_provider.fetch_images(kw, per_page=target_count, orientation=pixabay_orientation, negative_tags=negative_tags)
                if len(images) >= target_count:
                    break
        elif provider == "pexels":
            for kw in keywords:
                images += pexels_provider.fetch_images(kw, per_page=target_count, orientation=pexels_orientation, negative_tags=negative_tags)
                if len(images) >= target_count:
                    break
        return [u for u in images if u not in exclude_urls]

    with concurrent.futures.ThreadPoolExecutor(max_workers=len(providers)) as executor:
        futures = {executor.submit(_fetch_images, provider): provider for provider in providers}
        for future in concurrent.futures.as_completed(futures):
            provider = futures[future]
            try:
                images = future.result()
                for img in images:
                    all_images.append({"type": "image", "url": img, "provider": provider})
            except Exception as e:
                print(f"[ASSET ERROR] Error fetching images from {provider}: {e}")

    # For images, just return up to target_count
...
def get_images_for_scene(keywords: list[str], target_count: int = 4,
    ...
    أفقية. توزّع البحث بين Pixabay و Pexels.
    لا تستخدم كلمات احتياطية عامة — أي صورة تُعرض يجب أن تكون مرتبطة
    فعلاً بالنص السردي. لو لم نجد شيئاً، نرجع قائمة فارغة والمستدعي
    يقرر (إعادة توليد كلمات أو تمديد المشهد السابق).
    """
...
def get_media_for_scene(keywords: list[str], target_count: int = 1,
                         ...
                         topic_context: str = "", providers: list[str] = None,
                         exclude_urls: set = None, negative_tags: list[str] = None) -> list[dict]:
    """
    ...
    providers: لو مُمرّرة (مثلاً ["pixabay"]) تُقصر البحث على هذا المزود
    فقط بدل تجربة كل المزودين المتاحين — يستخدمها parallel_scene_fetcher.py
    للتحكم بأي مزوّد يُستخدم لأي مشهد بأي لحظة.

    exclude_urls: روابط سبق تجربتها ورُفضت لهذه الوحدة البصرية بالذات —
    تُستبعد من النتائج حتى لو رجعت من نفس المزود، حتى نحصل فعلاً على
    "وسيط آخر غير الذي جُلب أول مرة" بدل تكرار نفس الرابط المرفوض.
    ...
    لا تستخدم كلمات احتياطية عامة — أي مشهد يُعرض يجب أن يكون ناتجاً عن
    بحث مرتبط بالنص السردي فعلاً. لو لم نجد شيئاً، نرجع قائمة فارغة
    والمستدعي (parallel_scene_fetcher) يقرر الخطوة التالية.
    """
    providers = providers if providers is not None else _available_providers()
    if not providers:
    ...
    if prefer_video:
        media = _try_videos_across_providers(contextualized_keywords, target_count, providers, is_short=is_short,
                                              exclude_urls=exclude_urls, negative_tags=negative_tags)
