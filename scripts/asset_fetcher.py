...
def _videos_from_provider(provider: str, keywords: list[str], target_count: int, is_short: bool = True,
                           exclude_urls: set = None) -> tuple[list[dict], float]:
    ...
    if provider == "pixabay":
        ...
        for kw in keywords:
            vids = pixabay_provider.fetch_videos(kw, per_page=target_count, is_short=is_short)
    ...
    if provider == "pexels":
        for kw in keywords:
            vids = pexels_provider.fetch_videos(kw, per_page=target_count, is_short=is_short)
            vids = [v for v in vids if v["url"] not in exclude_urls]
            if vids:
                return vids, 1.0
        return [], -1.0
...
def _try_videos_across_providers(keywords: list[str], target_count: int, providers: list[str], is_short: bool = True,
                                  exclude_urls: set = None) -> list[dict]:
    for provider in providers:
        vids, score = _videos_from_provider(provider, keywords, target_count, is_short=is_short,
...
def _try_images_across_providers(keywords: list[str], target_count: int, is_short: bool,
                                  providers: list[str], exclude_urls: set = None) -> list[dict]:
    exclude_urls = exclude_urls or set()
    ...
    for provider in providers:
        ...
        if provider == "pixabay":
            ...
            for kw in keywords:
                ...
                if len(images) >= target_count:
                    break
        images = [u for u in images if u not in exclude_urls]
        if images:
