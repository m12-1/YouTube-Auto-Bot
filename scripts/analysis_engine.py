...
عمر العملية (process) كلها:
  ...
  - عامل Gemini: يأخذ أي مشهد لم يُحسم بعد من طابور مشترك (queue) ويحلله
    بنماذج Gemini الأربعة بالتسلسل (راجع gemini_client.score_media_relevance).
...
سلسلة التدرّج الكاملة (Cascade)، من الأفضل للأسوأ:
  الطبقة 1 أو 2 (بحسب أي عامل أمسك المشهد): Gemini Vision (4 نماذج) أو
...
class _VerifyJob:
    """طلب فحص مشهد واحد بانتظار أي عامل متاح يلتقطه من الطابور."""
    __slots__ = ("file_path", "narration", "event", "result")

    def __init__(self, file_path: str, narration: str):
...
def _resolve_job(lane_name: str, file_path: str, narration: str) -> bool:
    """يحل مصير مشهد واحد بعامل معيّن (lane_name)، بما فيه كل منطق
    التوقف/الانتظار/الاستبدال بـ Puter الموضّح بأعلى الملف."""
...
def _lane_worker(lane_name: str) -> None:
    ...
    while True:
        ...
        try:
            job.result = _resolve_job(lane_name, job.file_path, job.narration)
        except Exception as e:
...
def verify(file_path: str, narration: str) -> bool:
    ...
    داخلياً: تضع هذا المشهد كطلب بطابور مشترك يعمل عليه عاملان دائمان
    (Gemini وGroq) بالتوازي طوال التشغيل — كل عامل يمسك مشهداً مختلفاً في
    نفس اللحظة بمجرد أن يصبح متاحاً، بدل ازدواج نفس المشهد بينهما. تُحظر
    (block) حتى يُحسم مشهدها تحديداً ثم ترجع True/False.
    """
    _ensure_lanes_started()
    job = _VerifyJob(file_path, narration)
