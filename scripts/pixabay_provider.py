...
def fetch_images(keyword: str, per_page: int = 3, orientation: str = "horizontal", negative_tags: list[str] = None) -> list[str]:
    ...
    try:
        ...
        valid_urls = []
        for h in hits:
            hit_tags = h.get("tags", "").lower()
            if negative_tags and any(nt in hit_tags for nt in negative_tags):
                continue
            valid_urls.append(h["largeImageURL"])
        return valid_urls
