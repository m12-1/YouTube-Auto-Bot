...
{{
  ...
  "scenes": [
    {{"scene_number": 1, "narration": "...", "visuals": [
        {{"keywords": ["concrete filmable phrase 1", "concrete filmable phrase 2"], "duration_seconds": 3.0}},
        {{"keywords": ["concrete filmable phrase 1", "concrete filmable phrase 2"], "duration_seconds": 2.5}}
    ]}},
    ...
  ],
  "closing_cta": "..."
...
{{
  ...
  "scenes": [
    {{"scene_number": 1, "narration": "single line narration here", "visuals": [
        {{"keywords": ["concrete filmable phrase 1", "concrete filmable phrase 2"], "duration_seconds": 2.0}}
    ]}},
    {{"scene_number": 2, "narration": "single line narration here", "visuals": [
        {{"keywords": ["concrete filmable phrase 1", "concrete filmable phrase 2"], "duration_seconds": 1.5}},
        {{"keywords": ["concrete filmable phrase 1", "concrete filmable phrase 2"], "duration_seconds": 1.5}}
    ]}}
  ],
...
def _normalize_visuals(scene: dict) -> list[dict]:
    ...
    if isinstance(raw_visuals, list) and raw_visuals:
        ...
        for v in raw_visuals:
            ...
            try:
                ...
                duration = 2.0
            cleaned.append({"keywords": keywords, "duration_seconds": max(0.3, duration)})
        if cleaned:
            return cleaned
    ...
    return [{"keywords": flat_keywords, "duration_seconds": estimated_duration}]
...
Suggest 3 NEW alternative concrete, filmable stock-footage search phrases (2-4 words each) that still represent the same idea in the narration above, but are DIFFERENT from the failed phrases and more likely to actually exist as real stock footage (prefer more common/generic real-world visuals with the same core meaning, e.g. if a very specific phrase failed, suggest a broader but still relevant one).
...
def suggest_replacement_visual(narration: str, failed_keywords: list[str]) -> list[str]:
    ...
    تستخدم الموديل الخفيف (key_type='light') لأنها استعلام قصير ومتكرر
    الاحتمال (قد يحدث لعدة مشاهد بنفس الفيديو) ولا يستدعي موديلاً متقدماً.
    ...
    try:
        raw = gemini_client.generate_text(prompt, key_type="light", json_mode=False, temperature=0.8)
