...
MEDIA_CONTRAST_MIN: float = _get_env_float("MEDIA_CONTRAST_MIN", 20.0)
MEDIA_COMPRESSION_ARTIFACT_THRESHOLD: float = _get_env_float(
    "MEDIA_COMPRESSION_ARTIFACT_THRESHOLD", 15.0
)
MEDIA_WATERMARK_CORNER_RATIO: float = _get_env_float(
    "MEDIA_WATERMARK_CORNER_RATIO", 0.15
)
...
MEDIA_RANK_WEIGHT_SHOT_DIVERSITY: float = _get_env_float(
    "MEDIA_RANK_WEIGHT_SHOT_DIVERSITY", 0.06
)
MEDIA_RANK_WEIGHT_VISUAL_CINEMATIC: float = _get_env_float(
    "MEDIA_RANK_WEIGHT_VISUAL_CINEMATIC", 0.06
)

# Shot type keywords used for shot diversity scoring in media_ranker.
# Each key is a canonical shot type; its value is a list of indicator
# phrases found in stock-provider tags/URLs.
MEDIA_SHOT_TYPE_KEYWORDS: dict = {
    "wide": ["wide shot", "wide angle", "establishing shot", "landscape", "panorama", "panoramic"],
    "medium": ["medium shot", "mid shot", "waist shot", "half body"],
    "close": ["close up", "closeup", "close-up", "portrait", "face", "detail", "headshot"],
    "macro": ["macro", "extreme close", "microscope", "microscopic", "magnified"],
    "aerial": ["aerial", "drone", "bird eye", "birds eye", "overhead", "top down", "top-down"],
}
...
# Target integrated loudness (LUFS) for narration normalization, applied via
# ffmpeg's loudnorm filter right after synthesis so every run's narration
# lands at a consistent, YouTube-appropriate loudness regardless of which
# random voice/rate/pitch combination was chosen. -14 LUFS matches YouTube's
# own approximate normalization target, so videos neither get turned down
# on playback (too loud) nor sound noticeably quieter than other content
# (too quiet). Disabled automatically if ffmpeg isn't on PATH.
VOICE_LOUDNESS_NORMALIZATION_ENABLED: bool = (
    _get_env("VOICE_LOUDNESS_NORMALIZATION_ENABLED", "true") or "true"
).lower() == "true"
VOICE_TARGET_LUFS: float = _get_env_float("VOICE_TARGET_LUFS", -14.0)
...
VIDEO_TRANSITIONS: List[str] = _get_env_list(
    "VIDEO_TRANSITIONS", ["cut", "fade", "crossfade", "dissolve"]
)
VIDEO_TRANSITION_DURATION_SECONDS: float = _get_env_float(
    "VIDEO_TRANSITION_DURATION_SECONDS", 0.5
)
