...
def run():
    ...
    try:
        ...
        for i, (keywords, timing, narration) in enumerate(zip(scene_keywords, scene_timings, scene_narrations)):
            ...
            # إذا فشلت جميع الخيارات في اجتياز الفلتر البصري، نستخدم صورة خلفية عامة لتجنب الشاشة السوداء
            if not local_path:
                print(f"[WARNING] رفض الفلتر البصري جميع وسائط المشهد {i}. سيتم استخدام صورة بديلة (Fallback).")
                fallback_url = "https://cdn.pixabay.com/photo/2020/09/23/19/40/gaming-5596956_1280.jpg" # صورة عامة عن الألعاب
                local_path = asset_fetcher.download_image(fallback_url, f"{WORKDIR}/short_scene_{i}_fallback.jpg")
                media_type = "image"

            if not local_path:
                # ملاذ أخير إذا فشل حتى التحميل الاحتياطي
                print(f"[ERROR] فشل تام في توفير وسائط المشهد {i}. سيتم تمديد المشهد السابق.")
                if media_items:
